...
startingpoints that the data may find itself in:
...
require a certain format (see below). To illustrate the workings of
these three functions, we will start by creating the different edgelists
(and nodelist) that the functions take as their input.
...
The function results in a nodelist with all of the concepts that feature
in the edgelist listed once. It also sets the value of the concepts by
default to positive/neutral (1) and add two empty columns: paradigms and
instruments. These columns are needed for other functions in the package
and may be filled in by the scholar (see next step). The returned
edgelist has two extra columns in comparison to the input: value.x and
value.y, these indicate the value of the 'from' and 'to' concepts as
listed in the nodelist (which were set to positive/neutral (1) by
default).

In the current format, the new edge and nodelist can be used with the
other functions in the cognitivemapr package. However, as no paradigms
and instruments are listed and the value of the concepts are set to
positive/neutral (1) by default, the scholar may still want to change or
add information to the nodelist.

## 5: Adding information & (re) aligning the edge and nodelist
