...
Policy by Femke Van Esch and Jeroen Snellens. For an instruction on how
to compile the data (edge and nodelists) needed to use the functions in
this package, see the 'Tutorial_edge_nodelist'.
...
[improve the functions along the lines of what Thomas did for the
software: 1) make it stop when a balance is reached 2) for cyclical
graphs, let the researcher chose what values to take for the unstable
concepts]
