...
test_node1[c("38"), c("id", "node_name","val_run1")] # make more elegant: how can I
# refer to a value in a row to show the row?
...
test_node2[c("38"), c("id", "node_name","val_run1")] # make more elegant: how can I
# refer to a value in a row to show the row?
```
...
However in order to reap the full benefits of CM analysis and grasp the relations between Rutte's ideas as well as reveal the argumentation and narrative underlying
his belief system, the CM is best analysed visually in its graphical form.

[Include a simple visualisation function - with values but not dimensions]
...
Then run the paradigm_support function with these parameters, and store this in a renewed node_measures df
...
#gives an error, tried to rewrite function into the following, but gives the same error. Working offline, so need to work on it again.
...
```{r}
# voor nu vb met andere colommen, later omzetten naar paradigm_a & paradigm_b

w_indegree <- sum(rutte_p2_node_measures$w_degree)
support_par1 <- sum(rutte_p2_node_measures$indegree)
support_par2 <- sum(rutte_p2_node_measures$w_indegree)
  
support <- c((support_par1/w_degree),(support_par2/w_degree))

data.frame(paradigms, support)
...
```{r}
# Laten staan, even bedenken hoe je dit nuttig in kunt zetten
...
In a similar way, but without the added complexity of integrating the idea of incommensurability in the analysis, the instrument_support function helps calculate the support for different types of policy instruments embedded in the CM. In order to use this function, the concepts in the CM need to be categorised as belonging to a certain type of policy instrument. In de example data for this package seven policy
instruments are distinguished: Stronger EU fiscal regulation, structural reforms, monetary
measures by the ECB, economic stimulation, fiscal support, financial
market measures and EMU reforms (Van Esch & snellens 2024). 
