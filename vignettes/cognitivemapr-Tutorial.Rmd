...
simple_cm <- draw_cm(rutte_p2_edgelist, rutte_p2_nodelist)

# I cannot seem to store this [JELLE?!]
# !!! hij heeft iets opgeslagen in het vignette, maar hoe en waarom??
pdf("simpel_cm.pdf")
plot(simple_cm)
dev.off()

# Save the graph as PNG
png("my_graph.png", width = 800, height = 800)
plot(graph)
dev.off()
...
interact_cm_lists <- prep_cm_visualisation(rutte_p2_edgelist,rutte_p2_node_measures)

# extract and store the new edge and node_measures list
interact_edgelist <- interact_cm_lists[[1]]
interact_node_measures <- interact_cm_lists[[2]]
```

Subsequently, the interactive_cm function will produce such an interactive CM for the researcher to analyse.

```{r}
interactive_cm_rutte_p2 <- interactive_visual_cm(interact_edgelist,interact_nodelist)

#krijg dit weer niet goed gereturned
```
